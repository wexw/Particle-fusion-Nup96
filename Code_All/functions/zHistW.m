...
function [xzP, yzP] = zHistW(superparticle, nbins)
    
%     figure;
    
%     [~,xzP,yzP] = histfit2_W(superparticle, nbins, 'kernel');
    [~,xzP,yzP] = histfit2W(superparticle, nbins, 'kernel');
    % plot settings
%     xlim([-100,100])
...
function [h,x,y] = histfit2W(data,nbins,dist)
...
try
    pd = fitdist(data,dist,'width',4);%0.00025
catch myException
...
% % % hh = bar(bincenters,bincounts,1);%,'FaceColor',[0 0.4470 0.7410]);
