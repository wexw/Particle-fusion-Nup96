...
nbins=150;
 ...
 for type=1:6
    ...
    min2=[];
    minp2=[];
     [min1,minp1]=min(U_All2{type,1});%#row of every colums
    [min2,minp2]=min(U_All2{type,1}');%#colum of every rpw
    rowsx=[];
    rowsx=unique(minp1);
    rowsy=[];
    rowsy=unique(minp2);
    rowsxs=round(median(rowsx));
    rowsys=round(median(rowsy));
    U_All{type,1}(rowsxs,rowsys);
    anglex3(type)=Anglex2{type,1}(rowsxs)
    angley3(type)=Angley2{type,1}(rowsys)
A=[]
A=U_All2{type,1};
columA=[];
rowA=[];
...
[~,shu]=min(rowA)%
[~,heng]=min(cloumA)
anglex3(type)=Anglex2{type,1}(1,shu)
angley3(type)=Angley2{type,1}(1,heng)

end
for type=1:6
    data2c=[];
    anglex4=anglex3(type)
    angley4=angley3(type)
SuperParticle0=StraightenNPC{type,1}';
SuperParticle1=SuperParticle0(:,1:3)';
SuperParticleX=[rotx(anglex4)*SuperParticle1];
SuperParticle=[roty(angley4)*SuperParticleX]';
    data2c=SuperParticle;
    data2c(:,4:5)=SuperParticle0(:,4:5);
    %
    sizeview=80;
    ...
    filename = ['figures/SideViewProjectionOfStraightenNPC' num2str(type) '.svg']; % 设定导出文件名
    ...
    pos = get(gcf,'Position');
    set(gcf,'PaperPositionMode','Auto','PaperUnits','inches','PaperSize',[pos(3), pos(4)])
    filename = ['figures/ZHistogramOfReconstruction' num2str(type) '.svg']; 
...
errorbar(xb,yb,stdyb,'o','MarkerSize',10,...
    'MarkerFaceColor',[1 0.38 0.01],'LineWidth',2,'Color',[1 0.38 0.01])
...
print(gcf,filename,'-dsvg','-r0')
save('results/C_Optimal_straighten_NPC.mat')
