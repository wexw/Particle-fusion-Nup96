...
%             meanNRs_SPA{type,1}(1,3:4)=meanNRs_SPA{type,1}(1,3:4);
            MS{type,j}= GenerateBindingSites_WithGlobalPhase4(meanNRs_SPA{type,1});
        else
            %         TV{i,j}=CRs_SymmetricEmitters{i,minp(i,j)};
%             meanCRs_SPA{type,1}(1,3:4)=meanCRs_SPA{type,1}(1,3:4)/180*pi;
            MS{type,j}=  GenerateBindingSites_WithGlobalPhase4(meanCRs_SPA{type,1});
        end
        ...
        if j==1
            ...
            filename = ['figures/RegisteredEmittersNR' num2str(type) '.svg']; % 设定导出文件名
        end
        if j==2
            FE{type,j}=CRs_FreeGMM{type,minp(type,j)}.mu';
            filename = ['figures/RegisteredEmittersCR' num2str(type) '.svg']; % 设定导出文件名
        end
...
%     set(gcf, 'InvertHardCopy', 'off');
    pos = get(gcf,'Position');
    set(gcf,'PaperPositionMode','Auto','PaperUnits','inches','PaperSize',[pos(3), pos(4)])
    filename=['figures/SideViewEmittersCompare' num2str(type) '.svg']
    view(0,15)
    %     if j==1
...
for type=1:6
    ...
    filename = ['figures/CompareSAndFModelswithDistance' num2str(type) '.svg']; % 设定导出文件名
...
save('results/J_SymmetricAndFreeEmitters.mat','MS','FE','Distance')
