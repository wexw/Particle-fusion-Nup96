...
%
%   where:            each element of g_c() is g(x,y;A)
%       X       
%       g_c(x_c,y_c;A) - vector function of ALL the measurements
%                   - a matrix of the form: [x_c.^2, x_c.*y_c, y_c.^2, x_c, y_c ]
...
%   Orientation = phi
%
%   DOWNLOADED FROM FILE EXCHANGE (fit_ellipse by Ohad Gal)
...
if (nargin>2) & ~isempty( axis_handle ) & (test>0)
    ...
    plot( rotated_ellipse(1,:),rotated_ellipse(2,:),'w','LineWidth',2);
%     
% %     plot horizontal ellipse
%         plot( ver_line(1,:)-X0,ver_line(2,:)-Y0,'r');
%         hold on
%         plot( horz_line(1,:)-X0,horz_line(2,:)-Y0,'r' );
%         hold on
%         plot( ellipse_x_r-X0, ellipse_y_r-Y0,'LineWidth',3 );
    axis equal
